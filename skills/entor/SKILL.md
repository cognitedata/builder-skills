...
description: >-
  ...
  lifecycle", "entor", "onboarding", "getting started", "first Flows app".
allowed-tools: Read, Glob, Grep, Bash
---

# Entor — Getting Started with Flows Apps
...
### CI/CD
For automated deployment on merge to `main`, add GitHub Actions workflows (see `references/cicd.md` in this skill for templates). Use a service account client ID + secret stored as repo secrets.
...
See [CDF SDK docs](https://cognitedata.github.io/cognite-sdk-js/) for the full API.

**Q: My app works locally but fails inside Fusion — why?**
Check `manifest.json`. If your app calls external APIs (anything other than the CDF cluster), those domains must be listed under `permissions.network`. The Cognite CLI's `migrate-app-to-flows` skill or `setup-flows-auth` skill can audit this for you.
...
**Q: Can I use a monorepo structure with both CDF Toolkit (backend) and a Flows app (frontend)?**
Yes — see the `toolkit-flows-app-template` for the recommended layout: `modules/` for CDF resources, `app/` for the Flows frontend.
...
# Deploy preview (no app launcher)
npx @cognite/cli@latest apps deploy --interactive
